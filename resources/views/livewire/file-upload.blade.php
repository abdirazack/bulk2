...
@if (empty($fileData))
 <button wire:click="$dispatch('openModal', { component: 'organization-user.create' })">Edit User</button>
            ...
            <div class="mt-6">
                ...
                <form wire:submit.prevent="saveModifiedData">
                    <table class="table table-compact table-zebra">
                        <thead>
                            <tr>
                                @foreach ($modifiedData[0] as $header)
                                    <th>{{ $header }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($modifiedData as $index => $row)
                                @if ($index > 0)
                                    <tr wire:key="{{ $index }}">
                                        @foreach ($row as $cell)
                                            <td>
