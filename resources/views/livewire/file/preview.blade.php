            ...
            <table class="table table-compact table-zebra">
                ...
                <tbody>
                    ...
                    <tr wire:key="{{ $index }}">
                        ...
                        @if ($cellIndex == 1)
                        <td>
                            <select class="select select-bordered w-full max-w-xs " wire:model.lazy="fileData.{{ $index }}.{{ $cellIndex }}">
                                <option value="">Select an option</option>
                                @foreach ($accountProviders as $accountprovider)
                                @php
                                @endphp
                                <option value="{{ $accountprovider }}" {{ $cell == $accountprovider ? 'selected' : '' }}>
                        ...
                        @else
                        <td>
                            <input type="text" contenteditable="true" class="input input-bordered w-full max-w-xs" wire:model.lazy="fileData.{{ $index }}.{{ $cellIndex }}">
                        </td>
                        @endif
                        @endforeach
                        <td>
